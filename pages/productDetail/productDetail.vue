<template>
	...
	<view class="page flex-col">
		...
		<view v-if="info.is_xunjia == 0" class="group_17 flex-row">
			...
			<view class="image-text_10 flex-col justify-around" @click="tabClick(33)">
				<text class="iconfont label_11" :class="{'tab_color' : collect == 33}">&#xe607;</text>
		...
		<view class="buy_block" v-if="buyBlock">
			...
			<view class="mode">
				...
				<view v-if="specifications !== undefined && specifications!=null && specifications.length > 0"
					class="specifications_block flex-col align-start">
					<view class="block_text">规格</view>
					<text class="specifications_text"
						:class="{'click_specifications_text' : specificationsIndex == index}"
						v-for="(item,index) in specifications" :key="index"
						@click="clickSpecifications(item,index)">{{item.name}}</text>
				</view>
