	...
	<view class="page">
		<view class="block_1 flex-col">
			...
			<view class="group_2 flex-col">
				...
				<view class="text-wrapper_1 flex-row align-center justify-between" @click="addressShow">
					<text class="text_2">所在地：<text v-for="(i,n) in address" :key='n'> {{i.text}} <text v-if="n != 2">/</text></text></text>
					<text class="iconfont icon_1">&#xe686;</text>
				</view>
			...
			<button class="button_4" form-type="submit">
				提交
			</button>
			</form>
				...
				<view class="mode">
					...
					<view class="choose-text flex-row justify-around">
						<view class="text-block flex-col align-center">
							<text :class="{'text-font_active':listIndex == item.id}" v-for="(item,index) in textList"
								:key="index" class="text-font"
								@click="chooseProduct(item.id,item.name)">{{item.name}}</text>
						</view>
						<view class="text-block flex-col align-center">
							<text :class="{'text-font_active':sListIndex == item.id}" v-for="(item,i) in sList[0].sub"
								:key="i" class="text-font"
								@click="chooseSproduct(item.id,item.name)">{{item.name}}</text>
						</view>
						<view class="text-block flex-col align-center">
							<text :class="{'text-font_active':tListIndex == item.id}" v-for="(item,i) in tList[0].child"
								:key="i" class="text-font"
								@click="chooseTproduct(item.id,item.name)">{{item.name}}</text>
						</view>
					</view>
			...
			<view v-if="fineness" class="fineness-block">
				...
				<view class="mode">
					...
					<view class="finess-text flex-row">
						<text v-for="(i,n) in finenessList" :key='n' class="text-block"
							@click="chooseFineness(i)">{{i}}</text>
					</view>
				</view>
			</view>
